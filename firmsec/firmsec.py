...
def build_markdown_report(findings, target, diff_data, compare):
    ...
    L += [
        ...
        f"> Generated: {now}  ",
        f"> Target: `{target}`  ",
        f"> Tool: FirmSec v1.1 (Axis OS Firmware Security Analyzer)",
        "",
    ]
    ...
    L += steps
    L += ["", "---", f"_Report generated by FirmSec v1.1 — {now}_"]
...
def parse_args():
    parser = argparse.ArgumentParser(
        prog="firmsec",
        description="FirmSec v1.1 — Axis OS Firmware Security Analyzer",
...
def main():
    ...
    if HAS_RICH:
        console.print(Panel.fit(
            "[bold cyan]FirmSec v1.1[/bold cyan] — Axis OS Firmware Security Analyzer\n"
            "[dim]Optimized for Axis OS | Mac + Kali Linux[/dim]",
            border_style="cyan"
        ))
    else:
        print("=" * 60)
        print("  FirmSec v1.1 — Axis OS Firmware Security Analyzer")
        print("=" * 60)
